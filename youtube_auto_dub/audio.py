...
def finalize_audio(
    ...
    When ``suppress_original`` is True the original dialogue track is NOT mixed
    back in — only the isolated music/effects bed (if available and requested)
    is added on top of the pure dub.  This is the correct mode for dubbing:
    the viewer should never hear the original speech.
    """
    ...
    if match_loudness:
        # When suppressing the original we match against the dubbed audio
        # itself rather than the source, so the loudness target reflects
        # the new dialogue level, not the original mix.
        ref = dubbed if suppress_original else original
        target = max(_loudness(ref), AUDIO_LOUDNESS_CEIL)
    ...
    if mix_ambient:
        ...
        # Only use a genuinely speech-free source: either the Demucs-separated
        # background stem, or the centre-channel-extracted bed. Never the
...
def render_video(
    ...
    """Render the final output, replacing original audio entirely with the dub.

    When a dub audio is provided it COMPLETELY REPLACES the original audio
    track — no mixing, no fallback to original. The ``-map 0:v:0 -map 1:a:0``
    ensures only the video from source and the dub audio are used.

    ``apad`` prevents cutting the last words: if the dubbed audio is slightly
    shorter than the video, silence is padded rather than truncating dialogue.
    The ``-t`` limit ensures we never exceed the video duration.
    """
    ...
    if dub_audio_path:
        # Replace audio entirely. apad prevents cutting the last words;
        # -t limits to video duration to prevent trailing silence.
        # Get video duration for precise limiting.
        video_dur = _get_duration(video_path)
        cmd.extend([
            "-c:a", FFMPEG_AUDIO_CODEC,
            "-map", "0:v:0", "-map", "1:a:0",
            "-af", "apad",
            "-t", f"{video_dur:.3f}" if video_dur > 0 else "0",
        ])
    else:
        cmd.extend(["-c:a", "copy"])
