...
يقدم هذا المستودع خط إنتاج قابلًا للتشغيل من GitHub Actions يبدأ من ملف فيديو يحتوي على كلام عربي، ثم يستخرج الصوت، ويفصل الكلام عن الخلفية عند تفعيل Demucs، ويُفرّغ الكلام مع التوقيت، ويترجم المقاطع، ويولّد الصوت الإنجليزي عبر VoxCPM، ويطبق Seed-VC لنقل هوية الصوت، ويضبط التوقيت، ثم يعيد تركيب الفيديو ويكتب تقارير قابلة للقراءة الآلية. مزامنة الشفاه عبر Wav2Lip مرحلة اختيارية مشروطة بوجود checkpoint موثق وبنجاح بوابة الوجه.
...
افتح تبويب **Actions**، اختر **Run YouTube Auto Dub** ثم **Run workflow** على الفرع `arena/01a03969-prostudio`. اختر `dub` في خانة المهمة، وضع مسار الفيديو داخل المستودع مثل `samples/comingstarr_arabic_source.mp4`. للإعداد الكامل اختر اللغة المصدر `ar`، اللغة الهدف `en`، محرك `voxcpm`، وفعل `diarize` و`separate_sources` و`bg_music`. فعّل `lip_sync` فقط بعد تجهيز checkpoint موثق.
...
الملف الأساسي هو `config/pipeline_profile.yaml`. الوضع `safe` يعطل الفصل والـ diarization وlip-sync ولا يعيد مزج الصوت الأصلي. الوضع `high_quality_single` يستخدم VoxCPM وSeed-VC مع مرجع متحدث واحد. الوضع `multi_speaker_cinematic` يفعّل المراحل الاختيارية، لكنها لا تتجاوز بوابات الصلاحية. الوضع `experiment` يحتفظ بتجارب Wav2Lip/Diff2Lip منفصلة ولا يستبدل الناتج المعتمد تلقائيًا.
